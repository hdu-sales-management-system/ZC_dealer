...
axios.get('http://mock.yidafu.org/mock/5bed11641171b67e95b2131d/presents').then(function (response) {
  data_list=response.data.presents;
...
const mockPromise = (callback) => {
  return new Promise(resolve => {
    setTimeout(callback, 500, resolve);
  });
};
...
if (!queryObj.page) {
  ...
  if (!queryObj.pageSize) {
    queryObj.pageSize = length(data_list);
  }
    ...
    schema.forEach((column) => {
      ...
      switch (column.showType) {
        ...
        case 'textarea':
          ...
          break;
        case 'image':
          record[column.key] = mockImage(column,i);
          break;
        case 'file':
        ...
        default:
          switch (column.key) {
            case 'id':
              record[column.key] =data_list[i]["id"];
              break;
            case 'title':
              record[column.key] =data_list[i]["title"];
              break;
            case 'description':
              record[column.key] =data_list[i]["description"];
              break;
            case 'price':
              record[column.key] =data_list[i]["price"];
              break;
            case 'hot':
              record[column.key] =data_list[i]["hot"];
              break;
            case 'commentCount':
              record[column.key] =data_list[i]["commentCount"];
              break;
            default:
              logger.error('unsupported dataType %s', column.key);
          }
      }
    });
    tmp.push(record);
    result.total=data_list.length;
  }
...
const testImageArray = data_list[2];
// 模拟图片数据

const mockImage = (field,num) => {
  // 返回的是array还是string?
  if (field.max > 1) {
    const mockResult = [];
    for (let i = 0; i <= num; i++) {
      mockResult.push(data_list[num]["cover"]);
    }
    return mockResult;
  } else {
    return data_list[num]["cover"];}
};
